...
const SignupForm: React.FC<SignupFormProps> = ({ onSuccess, onSwitchToLogin }) => {
  ...
  return (
    <Card className="w-full max-w-md mx-auto glass-morphism border-white/20 shadow-2xl">
      ...
      <CardContent className="space-y-6">
        <form onSubmit={handleSignup} className="space-y-6">
          ...
          <div className="space-y-2">
            <Label htmlFor="desiredRole" className="text-white font-medium">I want to join as *</Label>
            <Select value={desiredRole} onValueChange={setDesiredRole}>
              <SelectTrigger className="h-12 bg-white/10 border-white/20 text-white focus:bg-white/20 focus:border-purple-400 transition-all duration-300">
                <SelectValue />
              </SelectTrigger>
          ...
          <Button 
            type="submit" 
            disabled={isLoading} 
            className="w-full h-12 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 btn-hover"
          >
