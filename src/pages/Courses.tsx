...
const CourseCard = ({ course }: { course: typeof courses[0] }) => (
  <Card className="overflow-hidden bg-black/40 border border-white/10 backdrop-blur-md shadow-lg hover:shadow-purple-500/20 transition-all duration-300 transform hover:-translate-y-1">
    ...
    <CardFooter className="border-t border-white/5 pt-4 flex justify-between">
      <Button variant="outline" className="border-purple-500/50 text-purple-200 hover:bg-purple-900/30">
        Preview
      </Button>
      ...
      <div className="mb-8">
        <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-black/30 border border-white/10">
            <TabsTrigger value="all" className="data-[state=active]:bg-purple-800/50">All Courses</TabsTrigger>
            <TabsTrigger value="beginner" className="data-[state=active]:bg-purple-800/50">Beginner</TabsTrigger>
            <TabsTrigger value="intermediate" className="data-[state=active]:bg-purple-800/50">Intermediate</TabsTrigger>
            <TabsTrigger value="advanced" className="data-[state=active]:bg-purple-800/50">Advanced</TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
